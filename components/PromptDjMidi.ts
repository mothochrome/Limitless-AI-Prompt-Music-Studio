...
export class PromptDjMidi extends LitElement {
  ...
  private startChaos() {
    if (this.chaosIntervalId) return;
  
    this.chaosIntervalId = window.setInterval(() => {
      const newPrompts = new Map(this.prompts);
      const allPrompts = [...newPrompts.values()];
      ...
      // Pick 2 to 4 random prompts to activate
      const promptsToActivate = allPrompts
        .sort(() => 0.5 - Math.random())
        .slice(0, Math.floor(Math.random() * 3) + 2);
  
      for (const prompt of promptsToActivate) {
        // Give a random weight between 0.5 and 1.5
